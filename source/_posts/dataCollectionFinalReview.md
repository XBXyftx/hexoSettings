...
应用场景：
- 老人佩戴手环（半无源标签）
- 进入区域时，125 kHz读取器激活标签
- 标签用2.45 GHz向基站回传位置信息
```
...
| 算法 | 原理 | 效率 |
|------|------|------|
| **纯ALOHA** | 标签随时发送，碰撞后随机延时重发 | 18% |
| **时隙ALOHA** | 将时间划分为时隙，标签只在时隙开始时发送 | 37% |
| **帧时隙ALOHA** | 阅读器指定帧长，标签随机选择时隙 | 可调优 |
...
| **UWB** | 27 Mbps | 10 m | 10 mW | **厘米级定位**★ | 室内精确定位、AirTag |
...
| 算法 | 原理 | 优点 |
|------|------|------|
| **LEACH** | 轮换簇头，均衡能耗 | 延长网络寿命★ |
| **PEGASIS** | 链式结构，逐级聚合 | 减少通信量 |
...
A ──→ C ──→ D ──→ E ──→ B
  25m   25m   25m   25m
     能耗 ∝ 4 × 25³ = 4 × 15,625 = 62,500

结论：多跳比单跳节能 1,000,000/62,500 ≈ 16倍！★
```
