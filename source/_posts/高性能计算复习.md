...
这道题是 Cache 性能分析的经典综合题，核心在于正确运用**平均访问时间（AMAT）**公式和**含存储停顿的 CPU 执行时间**公式。需要注意的是，组相联 Cache 由于多路选择器的延迟，会同时影响**时钟周期**、**命中时间**和**失效开销**的绝对时间值。
...
$$
\text{AMAT} = \text{命中时间} + \text{失效率} \times \text{失效开销}
$$

**2. 每条指令的存储停顿周期**

$$
\text{存储停顿/指令} = \text{访存次数/指令} \times \text{失效率} \times \text{失效开销（周期数）}
$$

**3. CPU 执行时间**

$$
T_{\text{CPU}} = N \times (\text{理想CPI} + \text{存储停顿/指令}) \times T_{\text{clk}}
$$

其中 $N$ 为指令数，$T_{\text{clk}}$ 为时钟周期时间。

---

##### 第一步：确定 256 KB Cache 的失效率

题目给出的失效率对应 **$128\ \text{KB}$** Cache，而实际使用的是 **$256\ \text{KB}$**。在计算机系统结构中，Cache 容量加倍时，失效率通常按经验规律降低为原来的约 $0.7$ 倍（具体系数因程序和 Cache 结构而异，此处采用课程常见经验值）。

$$
\text{失效率}_{\text{直接映像}} = 1.25\% \times 0.7 = 0.875\%
$$

$$
\text{失效率}_{\text{组相联}} = 1\% \times 0.7 = 0.7\%
$$

> 若题目或课程中允许直接使用 $128\ \text{KB}$ 的失效率数据，只需将后续计算中的失效率替换为 $1.25\%$ 和 $1\%$ 即可，计算逻辑完全相同。
...
$$
\text{AMAT}_{\text{直接映像}} = 2 + 0.00875 \times 120 = 2 + 1.05 = 3.05\ \text{ns}
$$

也可以用时钟周期表示：

$$
\text{AMAT}_{\text{直接映像}} = (1 + 0.00875 \times 60) \times 2 = 1.525 \times 2 = 3.05\ \text{ns}
$$

**(2) 存储停顿周期/指令**

$$
\text{停顿}_{\text{直接映像}} = 1.3 \times 0.00875 \times 60 = 0.6825\ \text{（周期/指令）}
$$

**(3) 实际 CPI**

$$
\text{CPI}_{\text{实际}} = 2.0 + 0.6825 = 2.6825
$$
...
$$
T_{\text{CPU}} = N \times 2.6825 \times 2 = N \times 5.365\ \text{ns}
$$
...
组相联 Cache 的多路选择器使 CPU 时钟周期增加 $10\%$：

$$
T'_{\text{clk}} = 2 \times 1.1 = 2.2\ \text{ns}
$$
...
$$
\text{AMAT}_{\text{组相联}} = 2.2 + 0.007 \times 132 = 2.2 + 0.924 = 3.124\ \text{ns}
$$

用时钟周期表示：

$$
\text{AMAT}_{\text{组相联}} = (1 + 0.007 \times 60) \times 2.2 = 1.42 \times 2.2 = 3.124\ \text{ns}
$$

**(2) 存储停顿周期/指令**

$$
\text{停顿}_{\text{组相联}} = 1.3 \times 0.007 \times 60 = 0.546\ \text{（周期/指令）}
$$

**(3) 实际 CPI**

$$
\text{CPI}'_{\text{实际}} = 2.0 + 0.546 = 2.546
$$

**(4) CPU 执行时间**

$$
T'_{\text{CPU}} = N \times 2.546 \times 2.2 = N \times 5.6012\ \text{ns}
$$
...
| :--- | :--- | :--- |
| **时钟周期** | $2\ \text{ns}$ | $2.2\ \text{ns}$ |
| **失效率** | $0.875\%$ | $0.7\%$ |
| **平均访问时间** | $3.05\ \text{ns}$ | $3.124\ \text{ns}$ |
| **实际 CPI** | $2.6825$ | $2.546$ |
| **执行时间（每指令）** | $5.365\ \text{ns}$ | $5.6012\ \text{ns}$ |

**结论分析：**

- **平均访问时间**：直接映像 Cache 略优（$3.05\ \text{ns} < 3.124\ \text{ns}$）。这是因为虽然组相联的失效率更低，但多路选择器带来的 $10\%$ 时钟周期增加，使得命中时间的绝对值变大，抵消了部分失效率降低的收益。
- **实际 CPI 与执行时间**：直接映像 Cache 仍然占优（$5.365\ \text{ns} < 5.6012\ \text{ns}$）。在本题的参数设定下，组相联降低失效率带来的存储停顿减少（$0.6825 \to 0.546$），不足以弥补时钟周期拉长 $10\%$ 对理想执行时间的影响（$4.0\ \text{ns} \to 4.4\ \text{ns}$）。

> 但这并不意味着组相联总是更差。如果失效率差距更大、或者时钟周期增加幅度更小，组相联完全可能反超。本题的关键在于掌握**系统化的计算方法和比较思路**，而非死记硬背哪种结构更好。

#### 直接映像Cache和两路组相联Cache
