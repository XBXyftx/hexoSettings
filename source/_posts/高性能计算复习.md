...
- **Cache（SRAM）**：使用**六晶体管（6T）单元**构成一个存储位，不需要刷新，读写速度快，但集成度低、功耗高、成本高。因此只适合做小容量高速缓存（KB 到 MB 级别）。
- **内存（DRAM）**：使用**单晶体管+电容（1T1C）**结构，依靠电容电荷存储数据，需要定期刷新（因为电容会漏电），读写速度比 SRAM 慢一个数量级，但集成度高、成本低，因此可以做成大容量（GB 级别）。
...
易失性存储（寄存器、Cache、内存）依赖**持续供电**来维持数据，断电后数据立即丢失；非易失性存储（磁盘、Flash、SSD、光盘）通过**磁化方向**、**浮栅电荷**或**物理凹凸**来保存数据，断电后数据仍能长期保持。
