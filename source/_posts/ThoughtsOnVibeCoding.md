...
曾老师组织的VibeCoding训练营的的确确让我打开了潘多拉的魔盒，Codex，Claude Code，Gimini这些产品开始对我的开发模式进行彻底的重构，也对我对于学弟学妹的编程教育理念产生了深远的影响，我固然知道AI的重要以及VibeCoding的潜力之大，但每当我重新审视自己的发展历程以及对AI产品的认知的时候都会发现，我能够如此迅速的进行转变，最重要的两个因素，一个是精神上的热爱，另一个则是对于编程基础的掌握。

计组、数据结构让我理解了底层原理，让我虽然并不开发C/C++但是我能理解底层的指针、基础复杂类型、计算精度问题、深浅拷贝等问题。计网让我能在前后端联调测试中排查常见的问题，像是本地测试但是不在同一个子网导致链接不上；后端服务器带宽限制带来的502；数据编码格式不同啊之类的常规问题。

向上抽象，我的编程起点是看黑马学语言基础，这是近代程序员的常见起点————从任何一门语言的Hello World开始。然后逐步的学到各种各样的“接口”，也是一个越来越抽象的过程。

到现在，这个“抽象塔”已经被AI堆砌到了一个史无前例的高度。
...
在我接触编程或者说是我的学长们接触编程的时候都是从当时的最高级抽象高级编程语言或是低代码编程，像是scrach。

![34](ThoughtsOnVibeCoding/34.webp)

能先跳过底层如此之多的抽象层级，直接去学习最高级的抽象层级是因为下层建筑是紧密咬合，拥有确定性的链路，相同的程序在正确的环境下是能得到相同的效果和结果的。但如今的AI，是以一种过去无法想象的方式将高级编程语言进一步的抽象成了自然语言编程，但这一层的抽象并不具备确定性，相同的模型和相同的提示词，其产生的结果千差万别，尤其是在编程上，这种不确定性对于自身没有对项目有深刻理解，或是没有能力去了解项目真实实现细节的新生和小白来说，这层抽象是致命的，看似强大的编程能力实际上是完全依赖模型能力的提升，毫无不可替代性的能力。生成的页面比不过学设计，学美术用语言描述出的页面美观；生成的算法，优不过研究新算法的算法工程师；AI带来的样样皆通的副作用就是样样不精。

当然请大家不要误解我上面的这段话，我说的都是针对完全不会编程的小白，他们需要以此为生，要进入这个行业，他们早期的学习路径上就不能形成对AI的过度依赖。而对于不用以此为生，仅仅是想借助AI去让自己的想法落地的人来说，那自然是用得越多越好。
