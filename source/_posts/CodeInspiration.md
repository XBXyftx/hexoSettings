...
但这个想法很快就陷入了瓶颈期，更恰当地说，压根就没有出生在那段时间。因为我并不清楚我该如何去进行合理的MCP调用，我不知道如何把华为官方文档的内容给做成数据库，然后让AI去学习，或者说是读取。我从这个应用的完美形态和基础的代码两边向中间聚合进行双向编程的构想的时候，发现我聚合不了，双向编程被一块知识的缺失阻断了，应用需求的实现被一个框架能力的缺失所阻断了。这个时候先诞生出来的想法很显然是习惯性的畏难心理，但这个时候孙妈出现了，他掏出了一个东西
