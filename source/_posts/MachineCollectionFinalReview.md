...
**序列<{3} {4}>的含义**：事件3先出现，然后事件4在之后的某个时刻出现（可以不紧邻）。

**检查每个对象是否包含该序列**：

**对象A**：<{1,2,4} {2,3} {5}>
- 寻找3：在Timestamp 2找到{2,3}
- 寻找4（在3之后）：Timestamp 3是{5}，无4 ✗
- 注意：虽然Timestamp 1有4，但它在3之前，不符合要求
- **不包含<{3} {4}>**

**对象B**：<{1,2} {2,3,4}>
- 寻找3：在Timestamp 2找到{2,3,4}
- 寻找4（在3之后）：同一时刻的{2,3,4}中也有4
- 注意：虽然3和4在同一时刻，但由于4的位置不早于3，可以认为满足"3之后有4"
- **包含<{3} {4}>**
...
- 寻找4（在3之后）：
  - 同一时刻的{3,4}中有4 ✓
  - 或者Timestamp 3的{4,5}中也有4 ✓
- **包含<{3} {4}>**

**对象E**：<{1,3} {2,4,5}>
- 寻找3：在Timestamp 1找到{1,3}
- 寻找4（在3之后）：Timestamp 2的{2,4,5}中有4 ✓
- **包含<{3} {4}>**

**统计结果**：
- 包含<{3} {4}>的对象：B, C, D, E（共4个）
- 总对象数：5个

$$\text{Support}(<\{3\} \{4\}>) = \frac{4}{5} = 0.8$$
...
|---------|------|-----------------|-----------|--------|
| <{3, 4}> | 3和4同时出现 | B, C, D | 3/5 | **0.6** |
| <{3} {4}> | 3先出现，4后出现 | B, C, D, E | 4/5 | **0.8** |

**关键区别**：
- <{3, 4}>要求3和4在**同一时刻**出现，条件更严格
- <{3} {4}>只要求3在4**之前或同时**出现，条件更宽松
- 因此<{3} {4}>的支持度（0.8）高于<{3, 4}>的支持度（0.6）
...
1. **约束条件不同**：
   - <{3, 4}>：要求3和4必须同时出现（更严格）
   - <{3} {4}>：只要求3在4之前或同时出现（更宽松）

2. **包含关系**：
   - 如果一个序列包含<{3, 4}>，则必然包含<{3} {4}>
   - 但包含<{3} {4}>的序列不一定包含<{3, 4}>
   - 因此：Support(<{3} {4}>) ≥ Support(<{3, 4}>)

3. **对象E的例子**：
   - E的序列：<{1,3} {2,4,5}>
   - 包含<{3} {4}>：3在Timestamp 1，4在Timestamp 2 ✓
   - 不包含<{3, 4}>：3和4不在同一时刻 ✗
