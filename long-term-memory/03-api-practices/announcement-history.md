...
> **当前状态（2026-08-06）**：公告历史时间轴已在本地实现并通过构建；顶部新增第 22 期文章目录 hideToggle 折叠分组公告，继续复用现有 `/imgs/gifs/1.webp`，不替换公告 GIF。当前公告数据尚未推送；形成提交后可补记 `source_commit`。
...
当前档案已在顶部追加第 22 期、2026-08-06 的文章目录 hideToggle 折叠分组公告：

- 文章目录新增折叠分组，hideToggle 折叠块内标题收纳进以块标题命名的可折叠目录项；
- 点击折叠组内目录链接自动展开对应折叠内容块并滚动到位；
- 滚动进入折叠区域时目录组自动展开并高亮当前小节，折叠箭头已对齐文字中线；
- 该公告继续复用 `/imgs/gifs/1.webp`，不新增或替换 GIF 资源；功能实现与验证见 [operation-log #52](../04-operations/operation-log.md#52--2026-08-06--文章目录-hidetoggle-折叠分组与点击展开联动)。

此前顶部公告为第 21 期、2026-07-21 的首页星空与文章阅读可读性升级公告：
...
首批从本地 `_config.butterfly.yml` Git 历史恢复 16 期公告；历史时间轴上线公告为第 17 期，图库重构公告为第 18 期，Markdown 工作台公告为第 19 期，首页夜空视觉公告为第 20 期，星空与可读性升级公告为第 21 期，本次目录折叠分组公告后共 22 期：
