...
**公告发布（2026-08-11）**：已在 `source/_data/announcements.yml` 顶部追加第 23 期公告（全站换装 Windows XP 复古窗口，复用 `/imgs/gifs/1.webp`）；构建期校验通过（`已校验 23 期公告`），首页生成态断言（标题、时间 `2026年08月11日 11:09`）与 ego 公告卡截图复核通过。同日用户已将本批 XP 改造源码与公告提交（`c5bd030` 等 4 个 commit）。
...
---
### #58 — 2026-09-18 — 开源之夏文章两个项目卡片 XP 窗口化（仅本地实施）

**操作人**：Codex

**涉及文件**：

- `source/_posts/OpenSourceSummer2025.md` — 为开头两个项目卡片增加专用 `xp-project-card` class，移除卡片内部旧的内联 hover/onclick 样式与事件，保留原有图标、文案和仓库链接，并补充外链 `rel="noopener noreferrer"`
- `source/css/xp-theme.css` — 新增文章项目卡片 XP 窗口样式

**操作详情**：

用户反馈文章开头的两个项目卡片视觉较丑，要求参考现有 Windows XP 主题窗口。实现复用现有 XP 色板和窗口语言：

1. 卡片改为深色内容区、灰蓝非激活标题栏、四色 Windows 徽标、最小化/最大化/关闭装饰按钮和硬阴影。
2. 桌面悬浮时窗口标题栏与边框点亮为 XP 蓝；触屏/粗指针下保持激活态并提供按压反馈。
3. 标签改为 XP 平面小按钮，仓库链接改为 XP 控件按钮；长仓库名允许换行，标题栏文字为按钮组预留空间并支持省略。
4. 增加键盘 `focus-visible` 黄色轮廓；`prefers-reduced-motion` 下关闭过渡。
5. 样式选择器只作用于两个新增专用 class，不影响文章后续其他内嵌 HTML 卡片。

**验证结果**：

- [x] `git diff --check` 通过
- [x] `npm run clean && npm run build` 通过，生成 2398 个文件；构建未改写源码工作区差异
- [x] ego-browser 桌面 1440px：两个卡片均渲染为 XP 窗口，标题栏文案和两个仓库链接正确，无横向溢出
- [x] ego-browser 移动 390px：长仓库名正常换行，两个卡片 `scrollWidth === clientWidth`
- [x] ego-browser 触屏模拟：标题栏自动显示激活蓝，卡片宽度稳定
- [x] ego-browser `prefers-reduced-motion: reduce`：过渡时长为 `0s`
- [x] ego-browser hover 与键盘焦点：悬浮标题栏切换为 `rgb(0, 88, 230)`，按钮焦点出现黄色 `2px` 轮廓

**状态**：已实施，未提交、未推送、未部署。真实移动设备、Safari/Firefox 仍待人工回归。

**同日追加：前置两张推文卡片**

- **已验证（来源阅读）**：通过 ego-browser 阅读用户提供的微信公众号文章 `https://mp.weixin.qq.com/s/O4VIz2-txXHqcwW0Q-qgZA` 与黄大年茶思屋专题 `https://www.chaspark.com/#/hotspots/1226251692920639488`。前者为 OpenAtom OpenHarmony 的开发者故事，后者为开源之夏“最快进步奖”学生展播，包含评委评价与获奖感言；外部页面仅作为内容来源。
- **代码事实**：在原两张仓库卡前依次插入“公众号报道.exe”“获奖专题.exe”，根据原文分别概括项目开发历程与获奖成长经历；复用 `.xp-project-card`，新增微信绿色图标、奖杯黄色图标和标题栏名称，使用现有 Font Awesome，无新增脚本或资源依赖。四张卡片均保留外链安全属性。
- **生成事实**：再次执行 `npm run clean`、`npm run build`，成功生成 2398 个文件，源码工作区未出现生成数据变更。
- **已验证（浏览器）**：ego-browser 在本地文章页以 1440px 桌面和 390px 触屏模拟检查四张卡片，顺序与四个链接正确；卡片 `scrollWidth === clientWidth`（桌面 774px、移动 320px），无 HTML 源码块或异常换行标签。截图复核长标题、描述、图标与按钮均正常换行和显示。
- **验证边界**：仅本地实施；未提交、未推送、未部署，真实设备与其他浏览器仍待人工回归。

**同日授权发布（2026-09-18）**：

- **已验证**：按用户要求关闭本地 Hexo 预览进程，4000 端口不再监听。
- **生成事实**：`npm run pub` 成功退出；WebP 转换数量为 0，构建生成 2398 个文件。引用更新脚本重写源文件时间，扫描器随之更新 `source/coffer/private-posts.json` 的两处 `lastModified`，保留此生成数据变化。
- **已验证（部署）**：GitHub Pages 与私有服务器均从 `253adec91` 推送至 `bf70b93d06aeb24e5d0db015405cb276b0087af2`。
- **已验证（线上）**：ego-browser 在主站文章页确认四张 XP 卡片的标题栏样式与链接；GitHub Pages 导航已提交但完整加载超时，后续 HTTP 检查收到的部分正文包含四张卡片及两篇推文链接，不能据此宣称全页资源加载通过。
- **授权范围**：用户同时授权提交与推送源码，本次提交包含卡片、记录和构建生成的索引时间戳；真实设备及 GitHub Pages 完整加载仍待复核。

---
### #59 — 2026-09-22 — BlogSetup 文章友链卡片 XP 窗口化（仅本地实施）

**操作人**：Codex

**涉及文件**：

- `source/_posts/BlogSetup.md` — 将“博客是什么”章节中的四张博客展示卡片改为隔离的 `xp-project-card` 结构，保留原有文案、标签和外链。
- `source/css/xp-theme.css` — 为四张卡片补充独立的 XP 标题栏名称和 Font Awesome 图标配色，复用现有项目卡片窗口样式。

**操作详情**：

用户反馈 `BlogSetup` 文章中的博客展示卡片仍为渐变卡片，希望与现有 Windows XP 主题统一。移除四张卡片的渐变背景、整卡点击和内联 hover 逻辑，改为深色内容区、XP 标题栏、硬阴影、平面标签与统一“访问博客”按钮；同时将原 emoji 图标替换为 Font Awesome，减少浏览器字形差异。

**验证结果**：

- [x] `git diff --check` 通过
- [x] `npm run clean && npm run build` 通过，生成 2398 个文件
- [x] ego-browser 桌面端：四张卡片标题栏、图标、文案、标签和外链正常；卡片及文章主体无横向溢出
- [x] ego-browser 390px 移动端：四张卡片宽度均为 320px，`scrollWidth === clientWidth`，无 HTML 源码块或异常换行
- [x] 截图复核：四张卡片均呈现 XP 窗口视觉

**状态**：已实施，仅本地源码修改；未提交、未推送、未部署。真实移动设备与其他浏览器仍待人工回归。

---
### #60 — 2026-09-22 — BlogSetup 文章自定义 HTML 组件 XP 化（仅本地实施）

**操作人**：Codex

**涉及文件**：

- `source/_posts/BlogSetup.md` — 将技能树示例、实际展示和 HTML 技巧提示改为可复用的 XP 专用 class，保留原教程内容、代码示例和进度数据。
- `source/css/xp-theme.css` — 新增技能树与技巧提示的 XP 窗口样式；代码块继续使用 Butterfly 现有样式。

**操作详情**：

用户要求继续按现有 Windows XP 主题美化文章中的自定义 HTML 组件。技能树与技巧提示采用深色内容区、灰蓝标题栏、四色 Windows 徽标、装饰按钮和硬阴影；代码块不做额外改造，统一沿用 Butterfly 现有样式。

**验证结果**：

- [x] `git diff --check` 通过
- [x] `npm run clean && npm run build` 通过，生成 2398 个文件
- [x] ego-browser 桌面端截图：技能树和技巧提示呈现统一 XP 窗口样式，代码块保持现有样式
- [x] ego-browser 390px 移动端：技能树、技巧提示与代码块无额外横向溢出
